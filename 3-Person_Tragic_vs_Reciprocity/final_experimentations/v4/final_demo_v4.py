...
from final_agents_v4 import StaticAgent, VanillaQLearningAgent, TrulyAdaptiveAgent
from final_simulation_v4 import run_pairwise_tournament, run_nperson_simulation
...
def plot_comparison(results, title, save_path):
    ...
    for label, metric, r, c in metrics:
        ax = axes[r, c]
        for agent_name_key, data in results.items():
            data_p, data_n = data
            plot_data = data_p if 'Pairwise' in label else data_n
            ql_id = next((k for k in plot_data if k.startswith(agent_name_key)), None)

            if ql_id:
                ax.plot(plot_data[ql_id][metric], label=agent_name_key, color=agent_colors.get(agent_name_key))
...
if __name__ == "__main__":
    ...
    output_dir = "adaptive_comparison_final"
    os.makedirs(output_dir, exist_ok=True)

    all_results = {}
    agents_to_test = {
        "Vanilla": VanillaQLearningAgent(agent_id="Vanilla_1"),
        "Adaptive": TrulyAdaptiveAgent(agent_id="Adaptive_1"),
    }
    ...
    plot_comparison(all_results, f"Vanilla vs. Truly Adaptive Agent {experiment_name}", plot_path)
