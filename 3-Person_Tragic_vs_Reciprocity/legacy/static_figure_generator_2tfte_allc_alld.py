...
# Special run with only 2TFT-E, AllC, and AllD agents
...
def setup_experiments():
    """Defines the experiment with only 2TFT-E, AllC, and AllD agents."""
    return {
        "2 TFT-E + 1 AllC + 1 AllD": [
...
def save_all_agent_data_to_csv(coop_data, score_data, mode_name, results_dir):
    """Save detailed agent cooperation and score data to CSV files."""
    for exp_name in coop_data.keys():
        clean_name = exp_name.replace(' ', '_').replace('+', '_plus_')
        ...
        # Get number of rounds from first agent
        first_agent_id = list(coop_data[exp_name].keys())[0]
        num_rounds = len(coop_data[exp_name][first_agent_id]['mean'])
        ...
        for round_num in range(num_rounds):
            ...
            for agent_id in coop_data[exp_name]:
                round_data[f'{agent_id}_coop_mean'] = coop_data[exp_name][agent_id]['mean'][round_num]
                round_data[f'{agent_id}_coop_std'] = coop_data[exp_name][agent_id]['std'][round_num]
            
            # Add scores
            for agent_id in score_data[exp_name]:
                round_data[f'{agent_id}_score_mean'] = score_data[exp_name][agent_id]['mean'][round_num]
                round_data[f'{agent_id}_score_std'] = score_data[exp_name][agent_id]['std'][round_num]
            
            rounds.append(round_data)
        
        # Create and save DataFrame
        df = pd.DataFrame(rounds)
        filename = f"{mode_name}_all_agents_{clean_name}_detailed.csv"
        filepath = os.path.join(results_dir, filename)
        df.to_csv(filepath, index=False)
        print(f"  - Saved detailed agent data: {filename}")


def plot_aggregated_results(data, title, smoothing_window=5, save_path=None):
    """Creates a plot showing mean and confidence intervals for TFT cooperation."""
    sns.set_style("whitegrid")
    fig, ax = plt.subplots(figsize=(10, 8), constrained_layout=True)
    fig.suptitle(title + " (100 runs with 95% CI)", fontsize=16, weight='bold')
    ...
    for exp_name, stats in data.items():
        ...
        ax.fill_between(rounds, stats['lower_95'], stats['upper_95'], 
                       alpha=0.3, label=f'{exp_name} 95% CI')
        
        # Plot mean
        ax.plot(rounds, stats['mean'], linewidth=2.5, 
               label=f'{exp_name} Mean', marker='o', markersize=2)
        
        # Plot smoothed mean
        smoothed_mean = pd.Series(stats['mean']).rolling(
            window=smoothing_window, min_periods=1, center=True).mean()
        ax.plot(rounds, smoothed_mean, linewidth=2, 
               linestyle='--', label=f'{exp_name} Smoothed')
    ...
    ax.set_ylabel("Avg. TFT-E Cooperation Rate", fontsize=12)
...
def plot_agent_scores(score_data, title, save_path=None):
    ...
    sns.set_style("whitegrid")
    fig, ax = plt.subplots(figsize=(10, 8), constrained_layout=True)
    fig.suptitle(title + " - Agent Scores (100 run average)", fontsize=16, weight='bold')
    
    for exp_name, exp_data in score_data.items():
        # Group scores by agent type
        agent_type_scores = {}
        for agent_id, scores in exp_data.items():
            # Extract agent type from ID (e.g., "TFT-E_1" -> "TFT-E")
...
def plot_all_agent_cooperation(coop_data, title, save_path=None):
    ...
    sns.set_style("whitegrid")
    fig, ax = plt.subplots(figsize=(10, 8), constrained_layout=True)
    fig.suptitle(title + " - All Agent Cooperation Rates (100 run average)", fontsize=16, weight='bold')
    
    for exp_name, exp_data in coop_data.items():
...
if __name__ == "__main__":
    ...
    results_dir = "results_2tfte_allc_alld"
    ...
    for name, agent_list in experiments.items():
        ...
        print(f"    Mean TFT-E cooperation rate: {np.mean(pairwise_tft_aggregated[name]['mean']):.3f} ± {np.mean(pairwise_tft_aggregated[name]['std']):.3f}")
    ...
    for name, agent_list in experiments.items():
        ...
        print(f"    Mean TFT-E cooperation rate: {np.mean(neighbourhood_tft_aggregated[name]['mean']):.3f} ± {np.mean(neighbourhood_tft_aggregated[name]['std']):.3f}")
    ...
    # Save detailed agent data
    save_all_agent_data_to_csv(pairwise_all_coop_aggregated, pairwise_scores_aggregated, "pairwise", results_dir)
    save_all_agent_data_to_csv(neighbourhood_all_coop_aggregated, neighbourhood_scores_aggregated, "neighbourhood", results_dir)

    # --- Generate All Plots ---
    print("\nGenerating and saving all plots...")
    
    # TFT-E cooperation plots
    plot_aggregated_results(pairwise_tft_aggregated,
                           title="TFT-E Cooperation Dynamics with Pairwise Voting",
                           save_path=os.path.join(results_dir, "pairwise_tfte_cooperation.png"))
    plot_aggregated_results(neighbourhood_tft_aggregated,
                           title="TFT-E Cooperation Dynamics with Neighbourhood Voting",
                           save_path=os.path.join(results_dir, "neighbourhood_tfte_cooperation.png"))
    
    # Agent scores plots
    plot_agent_scores(pairwise_scores_aggregated,
                     title="Pairwise Agent Scores",
                     save_path=os.path.join(results_dir, "pairwise_agent_scores.png"))
    plot_agent_scores(neighbourhood_scores_aggregated,
                     title="Neighbourhood Agent Scores",
                     save_path=os.path.join(results_dir, "neighbourhood_agent_scores.png"))
    
    # All agent cooperation plots
    plot_all_agent_cooperation(pairwise_all_coop_aggregated,
                              title="Pairwise All Agents",
                              save_path=os.path.join(results_dir, "pairwise_all_cooperation.png"))
    plot_all_agent_cooperation(neighbourhood_all_coop_aggregated,
                              title="Neighbourhood All Agents",
                              save_path=os.path.join(results_dir, "neighbourhood_all_cooperation.png"))
