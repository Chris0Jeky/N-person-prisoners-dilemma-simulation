"""
Main experimental framework for comparing Q-learning agent variants
Runs comprehensive simulations comparing different Q-learning strategies against various opponents
in both pairwise and n-person (neighborhood) prisoner's dilemma games
"""
...
def nperson_payoff(my_move, num_cooperators, total_agents):
    """Calculate payoff for n-person prisoner's dilemma"""
...
def run_pairwise_tournament(agents, num_rounds):
    """Run pairwise tournament between agents"""
    for agent in agents: 
        agent.reset()
...
def run_nperson_simulation(agents, num_rounds):
    """Run n-person neighborhood simulation"""
    for agent in agents: 
        agent.reset()
    history = {a.agent_id: {'coop_rate': [], 'score': []} for a in agents}
    coop_ratio = None
...
if __name__ == "__main__":
    ...
    ql_configs = {
        "Hysteretic": {"class": HystereticQLearner,      # Hysteretic Q-learner
