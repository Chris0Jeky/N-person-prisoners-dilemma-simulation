...
class StaticAgent:
    ...
    def choose_nperson_action(self, prev_round_group_coop_ratio):
        ...
        intended_move = None
        if self.strategy_name in ["TFT", "TFT-E"]:
            # This is the pTFT logic (probabilistic Tit-for-Tat)
        ...
        # Apply exploration for TFT-E AFTER deciding the intended move.
        # This ensures exploration happens in the neighbourhood scenario.
        if self.strategy_name == "TFT-E" and random.random() < self._get_current_exploration_rate():
            return 1 - intended_move
...
# (The plotting and data saving functions remain unchanged)
...
if __name__ == "__main__":
    ...
    results_dir = "results_3tfte_single_fixed"  # Changed directory name to avoid overwriting
    ...
    pairwise_coop, pairwise_scores = run_pairwise_simulation_extended(agents, NUM_ROUNDS)
    ...
    print(f"  - Final scores: {[int(scores[-1]) for scores in pairwise_scores.values()]}")

    # --- Run Single Neighbourhood Simulation ---
    print(f"\nRunning Neighbourhood simulation...")
    nperson_coop, nperson_scores = run_nperson_simulation_extended(agents, NUM_ROUNDS)
    ...
    print(f"  - Final scores: {[int(scores[-1]) for scores in nperson_scores.values()]}")
