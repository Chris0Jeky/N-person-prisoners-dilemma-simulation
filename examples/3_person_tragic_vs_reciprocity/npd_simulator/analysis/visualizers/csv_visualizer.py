"""
CSV-based visualizer for NPD simulation results
...
class CSVVisualizer:
    ...
    def visualize_experiment(self, experiment_path: Path) -> Dict[str, Any]:
        ...
        print(f"Generating visualizations for {experiment_path.name}...")
    ...
    def visualize_batch(self, batch_dir: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for exp_dir in batch_dir.rglob("csv"):
            if exp_dir.is_dir():
                ...
                self.figures_dir = parent_dir / "figures"
                self.figures_dir.mkdir(parents=True, exist_ok=True)
                
                # Generate visualizations
                plots = self.visualize_experiment(parent_dir)
                if plots:
                    results[parent_dir.name] = plots
        
        return results


def main():
    """Example usage of CSV visualizer"""
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python csv_visualizer.py <path_to_experiment_or_batch>")
        sys.exit(1)
    ...
    if (path / "csv").exists():
        ...
        # Batch directory
        visualizer.visualize_batch(path)
