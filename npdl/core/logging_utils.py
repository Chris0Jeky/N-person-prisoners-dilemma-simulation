...
def log_round_stats(
    round_num, agents, moves, payoffs, logger=None, logging_interval=10
):
    ...
    if q_learning_agents:
        ...
        for agent in q_learning_agents:
            for state in agent.q_values:
                if (
                    "cooperate" in agent.q_values[state]
                    and "defect" in agent.q_values[state]
                ):
        ...
        adaptive_agents = [
            a for a in agents if a.strategy_type == "q_learning_adaptive"
        ]
        if adaptive_agents:
            avg_epsilon = sum(a.strategy.epsilon for a in adaptive_agents) / len(
                adaptive_agents
            )
    ...
    if logger.level <= logging.DEBUG:
        ...
        for strategy, strategy_agents in strategies.items():
            ...
            strategy_coop_rate = (
                sum(1 for m in strategy_moves if m == "cooperate") / len(strategy_moves)
                if strategy_moves
                else 0
            )
...
def log_experiment_summary(
    scenario, run_number, agents, round_results, theoretical_scores, logger=None
):
    ...
    logger.info(
        f"  Theoretical Max Coop Score: {theoretical_scores['max_cooperation']:.2f}"
    )
    logger.info(
        f"  Theoretical Max Defect Score: {theoretical_scores['max_defection']:.2f}"
    )
    ...
    if q_learning_agents:
        ...
        if q_count > 0:
            ...
            avg_q_defect /= q_count
            logger.info(
                f"Final average Q-values - Q(c): {avg_q_coop:.2f}, Q(d): {avg_q_defect:.2f}"
            )
    ...
    if len(round_results) >= 3:  # Need at least 3 points to analyze a trend
        start_coop = sum(
            1 for move in round_results[0]["moves"].values() if move == "cooperate"
        ) / len(round_results[0]["moves"])
        middle_idx = len(round_results) // 2
        middle_coop = sum(
            1
            for move in round_results[middle_idx]["moves"].values()
            if move == "cooperate"
        ) / len(round_results[middle_idx]["moves"])
        end_coop = coop_rate
        ...
        logger.info(
            f"  Initial: {start_coop:.2f}, Middle: {middle_coop:.2f}, Final: {end_coop:.2f}"
        )
    ...
    if len(strategies) > 1:
        ...
        for strategy, strategy_agents in strategies.items():
            strategy_agent_ids = [a.agent_id for a in strategy_agents]
            strategy_moves = [
                last_round["moves"][aid]
                for aid in strategy_agent_ids
                if aid in last_round["moves"]
            ]
            strategy_coop_rate = (
                sum(1 for m in strategy_moves if m == "cooperate") / len(strategy_moves)
                if strategy_moves
        ...
        logger.info(
            f"Least cooperative strategy: {least_coop[0]} ({least_coop[1]:.2f})"
        )
    ...
    if q_learning_agents and len(round_results) > 1:
        ...
        if (
            "avg_q_coop" in locals()
            and "avg_q_defect" in locals()
            and abs(avg_q_coop - avg_q_defect) > 3.0
        ):
