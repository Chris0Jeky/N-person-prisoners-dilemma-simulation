...
class InteractiveGame:
    ...
    def _setup_game(self):
        ...
        self.payoff_matrix = create_payoff_matrix(
            self.num_agents, payoff_type=self.payoff_type, params=self.payoff_params
        )

        # Create environment
        self.env = Environment(
            self.agents, self.payoff_matrix, network_type=self.network_type
        )
    ...
    def _get_human_move(self):
        ...
        while True:
            choice = (
                input("Your move (c for cooperate, d for defect): ").strip().lower()
            )
            if choice == "c":
                return "cooperate"
            elif choice == "d":
    ...
    def _display_round_results(self, round_num, moves, payoffs):
        ...
        for neighbor_id in self.human_neighbors:
            ...
            print(
                f"Agent {neighbor_id} ({neighbor.strategy_type}): {neighbor_move.upper()} (payoff: {neighbor_payoff:.2f})"
            )
    ...
    def _display_game_summary(self):
        ...
        for i, agent in enumerate(sorted_agents):
            if agent.agent_id == self.human_id:
                ...
                print(
                    f"{i+1}. Agent {agent.agent_id} ({agent.strategy_type}): {agent.score:.2f}"
                )
        ...
        for agent in self.agents:
            agent_moves = [
                round_result["moves"][agent.agent_id] for round_result in self.history
            ]
            coop_rate = sum(1 for move in agent_moves if move == "cooperate") / len(
                agent_moves
            )
    ...
    def run_game(self):
        ...
        print(
            f"You'll be playing against {self.num_agents - 1} AI agents over {self.num_rounds} rounds."
        )
        ...
        for round_num in range(self.num_rounds):
            ...
            if round_num > 0:
                ...
                for neighbor_id in self.human_neighbors:
                    ...
                    print(
                        f"Agent {neighbor_id} ({neighbor.strategy_type}): {prev_move.upper()}"
                    )
            ...
            all_moves = {
                agent.agent_id: agent.choose_move(
                    self.env.get_neighbors(agent.agent_id)
                )
                for agent in self.agents
                if agent.agent_id != self.human_id
            }
            ...
            for agent in self.agents:
                ...
                neighbor_moves = {
                    n_id: all_moves[n_id] for n_id in neighbors if n_id in all_moves
                }

                # Update Q-values and memory (only for AI agents)
                if agent.agent_id != self.human_id:
                    agent.update_q_value(
                        all_moves[agent.agent_id],
                        payoffs[agent.agent_id],
                        neighbor_moves,
                    )
                agent.update_memory(
                    all_moves[agent.agent_id], neighbor_moves, payoffs[agent.agent_id]
                )

            # Store round results
            self.history.append(
                {"round": round_num, "moves": all_moves, "payoffs": payoffs}
            )
