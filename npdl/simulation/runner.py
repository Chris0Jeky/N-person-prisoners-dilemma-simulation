...
def setup_experiment(
    scenario: Dict[str, Any], logger: logging.Logger
) -> Tuple[Environment, Dict[str, float]]:
    ...
    if "agent_strategies" not in scenario:
        raise ValueError(
            f"Missing 'agent_strategies' in scenario: {scenario['scenario_name']}"
        )
    ...
    for strategy, count in scenario["agent_strategies"].items():
        for _ in range(count):
            agent_params = {
                ...
                "max_possible_payoff": scenario.get("payoff_params", {}).get(
                    "T", 5.0
                ),  # Pass T for optimistic init
            }

            # Add strategy-specific parameters
            agent_params.update(
                scenario.get("agent_params", {}).get(strategy, {})
            )  # Simplified generic param passing
            ...
            if strategy in [
                ...
                agent_params.update(
                    {
                        ...
                        "epsilon": scenario.get(
                            "epsilon", 0.1
                        ),  # Note: UCB1 ignores this internally
                        "state_type": scenario.get(
                ...
                if strategy == "ucb1_q":
                    agent_params["exploration_constant"] = scenario.get(
    ...
    payoff_matrix = create_payoff_matrix(
        scenario["num_agents"], payoff_type=payoff_type, params=payoff_params
    )
    ...
    if N_agents > 0:
        ...
        theoretical_scores["half_half"] = (n_coop * score_coop) + (
            n_defect * score_defect
        )
    else:
        ...
        if agent.strategy_type in (
            ...
            try:
                ...
                if state_count > 0:
                    ...
                    q_values_summary = {
                        "avg_cooperate": avg_coop_q,
                        "avg_defect": avg_defect_q,
                    }
                else:
                ...
                logger.error(
                    f"Agent {agent.agent_id}: Error processing Q-values for saving: {e}",
                    exc_info=False,
                )  # exc_info=False to avoid full stack trace unless needed
...
def print_comparative_summary(
    scenario_results_agg: Dict[str, Dict[str, Union[float, np.float64]]],
    logger: Optional[logging.Logger] = None,
) -> None:
    ...
    for scenario_name, agg_data in scenario_results_agg.items():
        ...
        avg_max_scores.append(
            agg_data["avg_final_max_score"]
        )  # Assuming max score per run is stored

    print(
        f"{'Scenario':<35} {'Avg Coop Rate':<15} {'Std Coop Rate':<15} {'Avg Max Score':<15}"
    )
    print("-" * 85)
    ...
    for i in range(len(scenarios)):
        ...
        coop_rate_str = (
            f"{avg_final_coop_rates[i]:.3f}" if i < len(avg_final_coop_rates) else "N/A"
        )
        coop_std_str = (
            f"{std_final_coop_rates[i]:.3f}" if i < len(std_final_coop_rates) else "N/A"
        )
        max_score_str = f"{avg_max_scores[i]:.2f}" if i < len(avg_max_scores) else "N/A"

        print(
            f"{scenario_name:<35} {coop_rate_str:<15} {coop_std_str:<15} {max_score_str:<15}"
        )
...
def run_simulation(
    enhanced: bool = False,
    scenario_file: str = "scenarios/scenarios.json",
    results_dir: str = "results",
    log_dir: str = "logs",
    analyze: bool = False,
    verbose: bool = False,
    ...
    for scenario in scenarios:
        ...
        for run_number in range(num_runs):
            ...
            try:
                env, _ = setup_experiment(
                    scenario, logger
                )  # Recalculates theoretical scores, but that's ok
            except (ValueError, KeyError) as e:
                logger.error(
                    f"Error setting up run {run_number} for scenario {scenario_name}: {e}"
                )
                continue
            ...
            try:
                ...
                logger.error(
                    f"Error saving results for {scenario_name} run {run_number}: {e}",
                    exc_info=True,
                )
            ...
            final_coop_rate = sum(
                1 for move in final_round["moves"].values() if move == "cooperate"
            ) / len(final_round["moves"])
            ...
            if strategies:
                avg_scores = [
                    (s, sum(a.score for a in ags) / len(ags))
                    for s, ags in strategies.items()
                ]
            ...
            logger.info(
                f"Completed Run {run_number} for {scenario_name} in {run_execution_time:.2f} seconds"
            )
        ...
        if scenario_run_results:
            ...
            all_max_scores = [
                r["final_max_strategy_score"] for r in scenario_run_results
            ]
        ...
        if analyze_multiple_scenarios is not None:
            try:
                ...
                analyze_multiple_scenarios(
                    scenario_names, results_dir, "analysis_results"
                )
                logger.info(
            ...
            logger.error(
                "Analysis module not available. Install npdl with analysis support."
            )
