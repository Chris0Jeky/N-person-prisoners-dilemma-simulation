        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1(
                            "N-Person Prisoner's Dilemma Visualization",
                                ...
                                dbc.Tab(
                                    [
                                        dcc.Loading(
                                            id="loading-payoffs",
                                            type="circle",
                                            children=[dcc.Graph(id="payoff-graph")],
                                        )
                                    ],
                                    label="Payoffs",
                                ),
                                dbc.Tab(
                                    [
                                        dcc.Loading(
                                            id="loading-scores",
                                            type="circle",
                                            children=[dcc.Graph(id="score-graph")],
                                        )
                                    ],
                                    label="Final Scores",
                                ),
...
@app.callback(
    [Output("run-dropdown", "options"), Output("run-dropdown", "value")],
...
def update_cooperation_graph(
    ...
    try:
        ...
        filtered_rounds = rounds_df[
            (rounds_df["round"] >= min_round) & (rounds_df["round"] <= max_round)
        ]

        # Filter by selected strategies
        filtered_rounds = filtered_rounds[
            filtered_rounds["strategy"].isin(selected_strategies)
        ]
        ...
        run_info = (
            f" (Run {run_value})"
            if run_value != "all" and run_value is not None
            else " (All Runs)"
        )
        fig.update_layout(
...
def update_payoff_graph(
    ...
    try:
        ...
        filtered_rounds["payoff"] = pd.to_numeric(
            filtered_rounds["payoff"], errors="coerce"
        )
...
def update_network_graph(scenario, run_value, round_num, n_clicks):
    ...
    try:
        ...
        G, network_data = load_network_structure(
            scenario, run_number=run_number, logger=silent_logger
        )
