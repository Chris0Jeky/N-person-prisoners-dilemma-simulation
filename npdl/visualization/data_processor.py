...
def get_payoffs_by_strategy(rounds_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        payoffs = (
            df.groupby(["round", "strategy"], dropna=True)["payoff"]
            .mean()
            .reset_index()
        )
        ...
        if payoffs.empty:
            logging.warning(
                "Empty result after groupby operation in get_payoffs_by_strategy"
            )
            return pd.DataFrame(columns=required_columns)
...
def get_strategy_scores(agents_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        scores = (
            df.groupby("strategy")["final_score"]
        ...
        if scores.empty:
            logging.warning(
                "Empty result after groupby operation in get_strategy_scores"
            )
            return pd.DataFrame(columns=["strategy", "mean", "std", "min", "max"])
...
def prepare_network_data(
    agents_df: pd.DataFrame, rounds_df: pd.DataFrame, round_num: Optional[int] = None
) -> Dict[str, Any]:
    ...
    if agents_df is None or rounds_df is None or agents_df.empty or rounds_df.empty:
        ...
        return {"nodes": [], "edges": [], "round": round_num or 0}
    ...
    if not all(col in agents_df.columns for col in required_agent_columns):
        missing = [
            col for col in required_agent_columns if col not in agents_df.columns
        ]
        logging.warning(f"Missing required columns in agents_df: {missing}")
        return {"nodes": [], "edges": [], "round": round_num or 0}
    ...
    if not all(col in rounds_df.columns for col in required_round_columns):
        missing = [
            col for col in required_round_columns if col not in rounds_df.columns
        ]
        logging.warning(f"Missing required columns in rounds_df: {missing}")
        return {"nodes": [], "edges": [], "round": round_num or 0}
    ...
    try:
        ...
        if round_data.empty:
            ...
            return {"nodes": [], "edges": [], "round": round_num}
